...
async function runMigration() {
  try {
    ...
    const migrationPath = path.join(__dirname, '../src/database/migrations/drop-admin-remarks-from-vendor-due-payments.sql');
    const sql = fs.readFileSync(migrationPath, 'utf8');

    console.log('\n📝 Running migration: drop-admin-remarks-from-vendor-due-payments.sql');
    ...
    try {
      ...
      if (err.message.includes('does not exist') || (err.message.includes('column') && err.message.includes('does not exist'))) {
        console.log('⚠️  Column does not exist, continuing...');
        successCount++;
      } else {
        throw err;
      }
    ...
    console.log('\n🎉 Admin remarks column dropped from vendor_due_payments table!');
